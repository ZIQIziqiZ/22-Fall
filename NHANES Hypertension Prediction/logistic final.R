...
# variables chosen with Forward Stepwise Regression
best_forward_revised <- readRDS('best_forward_revised.rds') #9544 * 11
BPX_I2015_2016 <- read.xport("BPX_I2015_2016.XPT") #systolic and diastolic blood pressure
BPX_I2015_2016 <- BPX_I2015_2016[c('SEQN','BPXSY1','BPXDI1')]
colnames(BPX_I2015_2016) <- c('seqn','bpxsy1','bpxdi1')
complete <- left_join(best_forward_revised, BPX_I2015_2016, by = 'seqn') #final dataset
...
# hypertension - 1; non-hypertension - 0
...
y_pred <- ifelse(probabilities > 0.2, 1, 0) #optimal threshold
...
# solution 1: use a subset of non-hypertension data
...
# solution 2: cluster the non-hypertension data first - k-means
...
## plot - how changes in threshold affect accuracy/sensitivity/specificity
